...
class PhotoModule extends React.Component {
  ...
  // Seed currently contains listingIds from 1-100 - make sure to use valid endpoint (e.g. "http://localhost:3003/rooms/3/").
  componentDidMount() {
    let listingId = window.location.pathname.split("/")[2];
    this.setSizeByScreen();
    fetch(`http://localhost:3003/rooms/1/images`)
      .then(res => res.json())
  ...
  setCaption() {
    ...
    let invisibleCaption = activePhoto.querySelector(".invisible-caption")
      .innerText;
  ...
  render() {
    return <div>{this.renderImgGrid()}</div>;
  }
}
